...
//
            ...
            self.commentclick = function(data, event) {
                ...
                var date=GetDateT();
                var data=self.data;
                data.push({name: name, time: date, comment: self.commentBoard()});
                $( "#listview" ).ojListView({ "data": new oj.ArrayTableDataSource(data)});
                var  comment={
                ...
                var details=self.commentBoard();
                self.commentBoard = ko.observable("");
                $.ajax({
                    type:"GET",
                    url:app.baseUrl+"comments/createComments/?email="+app.username+"&bugNumber="+self.bugNum+"&details="+details,
                    success:function(res){
                        app.gotoBugView(self.bugNum);
                    }
                });
                //self.dataSource=  new oj.ArrayTableDataSource(self.data, {idAttribute: "name"});
            }
